...
def get_details(id):
    sql = "SELECT f.id, f.name, f.description, f.year, c.name, l.name " \
          "FROM films AS f, countries AS c, languages AS l " \
...
def send(name, description, year, country_id, language_id):
    sql = "INSERT INTO films (visible, name, description, year, country_id, language_id) " \
          "VALUES (:visible, :name, :description, :year, :country_id, :language_id)"
    visible = 1
