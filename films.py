...
def get_list():
    sql = "SELECT f.id, f.visible, f.name, f.description, f.year, c.name" \
          "FROM films AS f, countries AS c WHERE c.id=f.country_id"
    result = db.session.execute(sql)
    return result.fetchall()

def get_details(id):
    sql = "SELECT f.id, f.name, f.description, f.year, c.name" \
          "FROM films AS f, countries AS c WHERE f.id=:id AND c.id=f.country_id"
    result = db.session.execute(sql, {"id":id})
    return result.fetchall()
...
def send(name, description, year, country_id):
    sql = "INSERT INTO films (visible, name, description, year, country_id)" \
          "VALUES (:visible, :name, :description, :year, :country_id)"
    visible = 1
