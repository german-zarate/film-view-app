...
def get_list(film_id):
    sql = "SELECT u.username, r.content, r.grade, " \
          "TO_CHAR(r.submitted, 'DD Month YYYY - HH24:MI'), c.name, c.code " \
          "FROM reviews AS r, users AS u, countries AS c " \
          "WHERE r.film_id=:film_id AND u.id=r.user_id " \
          "AND c.id=u.country_id AND u.banned=0"
...
def get_highest_rated():
    sql = "SELECT f.name, AVG(r.grade) AS average FROM films AS f, " \
          ...
          "AND r.user_id=u.id AND u.banned=0 GROUP BY f.id " \
...
def send(user_id, film_id, content, grade):
    sql = "INSERT INTO reviews " \
          "(user_id, film_id, content, grade, submitted) " \
          "VALUES (:user_id, :film_id, :content, :grade, NOW())"
    db.session.execute(sql, {"user_id":user_id, "film_id":film_id,
